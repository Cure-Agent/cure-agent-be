...
3. **BE 범위 판별**: 스펙은 이 레포에 있지만 FE 화면 스펙과 에이전트 서비스 스펙도 여기 산다. 수용 기준 중
   **이 레포가 책임지는 항목만** 고른다 — 판정은 항목 끝 괄호의 **레포 라벨**로만 한다(`docs/specs/TEMPLATE.md`).
   - 레포 라벨(`(BE …)`·`(FE …)`·`(AGENT …)`)이 달려 있으면 **`(BE …)` 항목만** 가져간다. `(FE …)`는
     `cure-agent-fe`, `(AGENT …)`는 `medical-agentic-rag`의 `/implement` 몫이다 — 여기서 구현하지도
     동결하지도 않는다.
   - 라벨이 **일부 항목에만** 있거나, 라벨은 없는데 범위 절에 `진입점 (FE — …)`·`진입점 (AGENT — …)` 표가
     있으면 **중단하고 사람에게 분할을 확인받는다.** 여러 레포에 걸친 스텝에서 라벨이 빠진 것은 스펙
     결함이다 — 추측으로 가르면 동결 범위가 실행마다 달라진다.
   - 라벨이 하나도 없고 다른 레포 표도 없으면 **전 항목이 BE다**(한 레포로 닫히는 스텝은 라벨을 생략한다).
     `(유닛)`·`(e2e — …)`처럼 검증 수단만 적힌 괄호는 레포 라벨이 아니다.
   > 이 단계가 없으면 BE 하네스가 남의 레포 항목까지 자기 몫으로 읽는다. 특히 `(AGENT)` 항목은 Python
   > 서비스의 동작이라 BE 테스트로 단언할 수조차 없고, 억지로 파생하면 스텁을 죽이지 못하는 공허 테스트나
   > 이 레포에 없는 구현을 요구하는 테스트가 동결된다.
4. `git status` clean 확인, `git checkout dev && git pull origin dev`.
5. **이슈 생성**: `gh issue create --title "[FEAT] <스텝명>" --label "✨ FEAT"` — 본문은
   `.github/ISSUE_TEMPLATE/기능-구현.yml`의 `body:` 각 항목 `label`을 마크다운 헤더(`## <label>`)로
   매핑하고 `required: true` 항목을 스펙의 범위·BE 범위 수용 기준(3)으로 채워 `--body`로 전달한다(폼 파일 자체를
   붙여넣지 않는다). 출력 URL 끝 숫자가 `<이슈번호>`다.
6. 브랜치 생성: `"feat/#<이슈번호>"` → checkout. 브랜치명에 `#`이 포함되므로 모든 git/gh 명령에서
   **반드시 따옴표로 감싼다**.
...
- **BE 범위**(Phase 0-3) 수용 기준 각 항목 ↔ 구현 파일·마이그레이션·에러코드 매핑 계획을 세운다.
...
| 파라미터 | 값 |
|---|---|
| 명세 | Phase 0에서 읽은 스펙의 **BE 범위 기준**(Phase 0-3) + §링크한 architecture.md 섹션 |
| 작업 ID | **이슈 번호** (Phase 0-5에서 발급 — 커밋 제목 `[TEST/#<이슈번호>]`에 쓰인다) |
| 동결 단위 | 스펙 1개 = 1 단위 |
| 참조 패턴 파일 | BE `test/auth.e2e-spec.ts` / FE `src/shared/api/http.test.ts` |

Codex 프롬프트에는 **BE 범위 기준만** 싣고, 다른 레포 기준은 번호와 함께 「이 레포 몫이 아니다 — 건드리지
마라」로 명시한다. 싣지 않으면 Codex가 FE 화면·에이전트 동작을 BE 테스트로 단언하려 시도하고, 번호를 비워
두면 빠진 번호가 누락인지 남의 몫인지 리뷰(`automation/freeze.md` 절차 4 ①)에서 가를 수 없다.

동결 후 구현이 테스트를 통과시키지 못하면 `automation/freeze.md`의 TEST-DISPUTE 3분류를 따르며, **분류에 따라 이 단계에서 멈출지가 갈린다**:

- **스펙 결함**(수용 기준이 모호·모순) → **Phase 1 규칙으로 회귀**한다. 스펙을 먼저 고치고 Codex 재파생 → 재동결, 사유는 커밋 메시지에 남긴다. 사용자 확인이 곧 스펙 확정 행위이므로 여기는 사람이 잡는다.
- **테스트 결함**(스펙은 옳고 테스트가 이를 잘못 대표) → **Codex 판정**에 넘긴다. 인정되면 Codex가 고친 개정본을 재검증 4계층(범위·스텁 재RED·본 브랜치 GREEN·결정성)에 걸고 재동결한 뒤 **Phase 1 회귀 없이 구현을 이어간다.** 판정 결과는 Phase 4 보고에 `⚠️ TEST-DISPUTE` 항목으로 싣는다.
- **구현 결함**(기본 추정) → 구현을 고친다.
...
3. BE 범위 수용 기준 항목별 → 커버하는 테스트 매핑을 만든다 (최종 보고에 포함). **다른 레포 몫으로 뺀
   항목 번호도 함께 적는다** — 매핑에 없는 번호가 누락인지 남의 몫인지 보고에서 구별돼야 한다.
4. 스펙의 Out of scope와 **다른 레포 범위 기준**을 침범하지 않았는지 점검한다.
5. **사용자 확인 대기** — 수용 기준 매핑·계약 변경 요약·**디스퓨트 판정 내역**(`⚠️ TEST-DISPUTE`: 테스트 이름 · 판정 · 근거 · 재검증 결과)을 보고하고 배포 승인을 받는다. 추가 수정 요청 시 반복. OK 시에도 **동결은 아직 해제하지 않는다**(배포 실패로 재수정하는 시나리오에서 테스트가 무방비가 되지 않도록).
