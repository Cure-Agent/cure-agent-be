...
1. 수용 기준의 **모든** 시나리오를 e2e 테스트로 작성한다 (`test/<도메인>.e2e-spec.ts`, Testcontainers 패턴은 `test/auth.e2e-spec.ts` 참고).
2. 컴파일에 필요한 최소 스텁(빈 컨트롤러·DTO 뼈대)은 허용하되 로직은 금지. 이 시점 테스트는 **실행 시 실패하는 것이 정상**이다.
3. 동결 커밋: `[TEST/#<번호>] <스텝명> 수용 기준 테스트 동결`
4. **이후 구현 중 동결 테스트 수정 금지.** 수정하고 싶어지면 그것은 스펙 결함 신호다 → Phase 1 규칙으로 회귀한다.
