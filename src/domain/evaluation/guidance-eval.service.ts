...
import { GUIDANCE_EVAL_PROFILES } from './guidance-eval.profiles';
...
export class GuidanceEvalService {
  ...
  /**
   * answerable 문항에 합성 프로필을 순환 배정해 케이스를 만든다 — 12종 프로필이 모두
   * 최소 한 번은 도는 결정적 배정이라 실행 간 비교가 성립한다.
   */
  async evaluate(
    items: EvalSetItem[],
    options: { limit?: number; profiles?: readonly PatientSnapshotPayload[] } = {},
  ): Promise<GuidanceEvalReport> {
    const profiles = options.profiles ?? GUIDANCE_EVAL_PROFILES;
    const answerable = items.filter((item) => item.kind === 'answerable');
    const limit = Math.min(options.limit ?? DEFAULT_CASE_LIMIT, answerable.length);
    ...
    for (let index = 0; index < limit; index += 1) {
      const item = answerable[index];
      const profile = profiles[index % profiles.length];
      try {
        const evaluated = await this.evaluateCase(item, profile);
  ...
  private async evaluateCase(
    ...
    const { answer, top } = await generateRealPathAnswer(
      { retrieval: this.retrieval, reranker: this.reranker, llmGateway: this.llmGateway },
      {
        ...
        promptQuestion: composeGuidanceQuestion(profile, item.question),
      },
    );
