...
export const ErrorCodes = {
  ...
  // 구성원 강퇴 (docs/specs/38) — 개설자가 자신을 내보내려는 경우.
  // `CLINIC_OWNER_MUST_TRANSFER`로 뭉치지 않는 이유는 **다음 행동이 다르기 때문**이다: 저것은
  // 「이양하면 탈퇴할 수 있다」이고 이것은 「강퇴로는 애초에 나갈 수 없다(이양 후 탈퇴하라)」다.
  // 문구가 그 차이를 담아야 사용자가 막힌 자리에서 다음 수를 안다.
  // `FORBIDDEN`(403)이 아닌 이유는 권한이 없어서가 아니라 **상태가 허락하지 않아서**다 —
  // 요청자는 개설자 권한을 갖고 있고, 그 권한으로도 자신은 뺄 수 없다.
  CLINIC_OWNER_CANNOT_REMOVE_SELF: {
    status: 409,
    message: '개설자는 자신을 내보낼 수 없습니다. 떠나려면 먼저 다른 구성원에게 권한을 넘기고 탈퇴해주세요.',
  },
