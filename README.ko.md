...
CureAgent는 임상지침 PDF 63개를 검색 가능한 근거 청크 7,154개로 변환하고, 가장 관련성 높은
...
- **하이브리드 검색** — pgvector 코사인 검색과 키워드 검색을 병렬로 실행하고, Reciprocal Rank
  Fusion으로 두 결과를 합친 뒤 LLM 리랭크를 적용한다. 키워드 arm은 역색인 어휘 위의 BM25 예산으로
  후보를 고른 뒤 `pg_trgm` 문자 n-gram 유사도로 순위를 매긴다.
...
오프라인 평가는 합성 검색 fixture가 아니라 프로덕션 코퍼스 스냅샷을 사용하며, 배포된 정책
(`hybrid-rrf60-top30x2-bm2575-cut0.48-score9-v6` · 생성 프롬프트 `qa-v6`) 그대로 돌린다.

| 범위                       |              규모 |
| -------------------------- | ----------------: |
| 조사한 임상지침 원문       |          PDF 87개 |
| 검색 코퍼스에 적재된 원문  |          PDF 63개 |
...
| -------------------------- | --------: |
| 하이브리드 후보 커버리지   |    100.0% |
| 키워드 arm Recall@30       |    100.0% |
| 리랭크 Recall@5            |     98.9% |
| 리랭크 MRR@5               |     0.942 |
| 범위 밖 문항 답변 수(누출) |    0 / 44 |
| 답변 가능 문항 답변 수     | 185 / 185 |
| 주장 단위 근거 지지율      |     91.8% |

기권은 end-to-end로 적는다 — 다층 게이트는 끝에서만 합쳐지기 때문이다. 배포된 관련도 컷에서 검색 단계
게이트가 범위 밖 44문항 중 41문항을 기권시키고(기권 재현율 93.2%), 남은 3문항은 생성 단계 답변가능성
게이트가 잡아 실제로 답한 문항은 없다. 답변해야 할 문항이 기권된 경우도 없다(과잉 기권율 0.0%). 검색
단계 기권 재현율은 생성 게이트 앞에서 센 값이라 안전 결과 자체가 아니라 누출의 상한이다.

검색과 기권 결과는 [2026-09-10 정책 실행](docs/rag-eval/2026-09-10-keyword-candidate-budget-v6.md),
주장 지지율은 [qa-v5 근거성 평가](docs/rag-eval/2026-08-03-groundedness-qa-v5.md)에서 가져왔다. 주장
지지율은 직전 생성 프롬프트에서 측정한 값이다. 229문항 평가셋은 컷오프 선정에도 사용됐으므로, 이 결과는
독립 held-out 일반화 벤치마크가 아니라 정책 진단 지표이며, 기권 44문항 축은 같은 구성을 반복 실행해도
1~2문항이 흔들린다. 프롬프트, 실패 사례, 분포 스윕, 평가 한계는 [전체 평가 리포트](docs/rag-eval/)를
참고한다.
