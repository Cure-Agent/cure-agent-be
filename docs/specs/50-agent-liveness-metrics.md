...
| 쟁점 | 판단 |
|---|---|
| 계측 범위 | **프로세스 메트릭만** — 공식 `prometheus-client`의 `make_asgi_app()`을 `/metrics` **정확 경로**에 붙인다(`app.mount()`가 아니다 — 아래 「범위(진입점)」의 정정 기록). 얻는 것은 `up`(스크레이프 성공 여부)과 `process_*`·`python_*`이고, RSS를 `mem_limit: 512m`과 대조할 수 있다. **도메인 라벨이 0개라 카디널리티·프라이버시 판단이 아예 없다.** §49 Out of scope 「에이전트 메트릭 — 기능이 생긴 뒤에 무엇을 셀지 정한다」는 그대로 유효하다: 이 스텝은 세는 것을 정하지 않고 **긁힐 수 있게** 만들 뿐이다 |
...
| `app/service/main.py` | 기본 레지스트리의 `make_asgi_app()`을 `/metrics` **정확 경로 Route**로 붙인다 — `app.mount()`는 쓰지 않는다. 라우터 접두사(`/api/v1/agent`) **밖**이다 |

> **정정 기록 (2026-09-13).** 이 표는 본래 「`make_asgi_app()`으로 `/metrics`에 마운트」라고 적었다.
> `app.mount("/metrics", …)`는 `/metrics` 자체를 매칭하지 않아 `/metrics/`로 **307**을 보내고
> (수용 기준 1이 요구하는 200이 아니다), `/metrics/<무엇이든>`까지 **200**으로 응답해 표면을 넓힌다
> — 위 「메트릭 경로」·「노출 표면」이 세운 방향과 반대다(starlette 실측). **수용 기준이 계약이므로**
> 구현은 정확 경로 Route로 착지했고(medical-agentic-rag `683620d`), 지시 문구를 그에 맞춰 고친다.
> 수용 기준 10의 「마운트」는 손대지 않는다 — 테스트가 이미 파생·동결됐고(`5dcdd0e`), 그 문구는
> 「ASGI 앱을 붙인 뒤」로 읽으면 그대로 성립한다. **운영 영향은 없었다**: 수집기는 리디렉션을 따라가므로
> 마운트로 구현했더라도 스크레이프 자체는 됐다. 문서 정확성의 문제다.
