...
`recordAbstain`을 부르지만, 생성 게이트(`:644`, `insufficient_evidence`)는 `recordGenerationGate`만
부르고 `recordAbstain`을 부르지 않는다. 「`recordAbstain` 옆에 저장한다」로 잡으면
**`insufficient_evidence` 행이 영원히 null로 남아** 이 스텝이 고치려는 바로 그 뭉뚱그림이 남는다.
...
**위험.** ⑴ 저장 지점이 둘이라 한쪽만 고치면 `insufficient_evidence`가 조용히 null로 남는다 —
기준 3이 이것만을 위해 존재한다. ⑵ ko 사용자에게 보이는 문장이 오늘과 달라진다(「보류했습니다」가
...
- **`recordAbstain('insufficient_evidence')` 누락 보정** — `abstain_total{reason}` 카운터가 생성 게이트
  기권을 세지 않는 것은 §40에서 넘어온 **별개의 관측 축 결함**이다. 이 스텝은 행 단위 사실을 만들 뿐
  집계 축을 건드리지 않는다 — 컬럼이 생기면 그 분포는 DB에서 직접 셀 수 있다
