...
생성)은 §19, 개정 감지 스케줄러는 별도 spec의 몫이다.
...
- **첨부가 없는 문서는 2,852B `text/html` 에러 페이지를 반환한다** (구판 계열. 최초 실측 86건 중 10건,
  2026-07-30 재실측 9건). 상태코드만으로 성공을 판정하면 HTML을 `.pdf`로 저장하게 된다.
- **상태 코드로 판정하지 않는다 — 본문을 받았으면 그대로 서비스에 넘긴다.** 이 응답의 상태 코드는
  최초 실측 시점의 `200 OK`에서 이후 **`500`으로 바뀌었다**(본문은 같은 2,852B 에러 페이지). 어댑터가
  `!response.ok`로 걸러내던 동안 그 변화만으로 아래 「첨부 없음」 판정 경로가 통째로 무력화돼, 9건이
  `SKIPPED_NO_ATTACHMENT`가 아니라 `FAILED`로 기록됐다(issue #102). 외부 서비스의 상태 코드는 언제든
  바뀔 수 있고 우리가 볼 수 있는 것은 본문뿐이므로, **판정 근거는 content-type과 매직바이트로 한정**한다.
  어댑터가 `GuidelineSourceError`를 던지는 경우는 **본문 자체를 받지 못했을 때**(네트워크 오류)뿐이다.
- 목록의 `guide_file`은 첨부 없는 문서에서 비어 있지만, **첨부 유무 판별에 쓰지 않는다.** 이 필드의
  용도는 저장 파일명이며(아래 「파일명」), 다운로드를 건너뛰면 본문 해시가 없어 아래 멱등 설계가
  깨진다 — 첨부 없는 문서도 HTML 본문의 해시를 가져야 재수집이 행을 늘리지 않는다.
...
- **개정 감지 스케줄러** (크론 + Redis 락) — 별도 spec. 수집 유스케이스가 안정된 뒤 그것을 감싸기만 한다
