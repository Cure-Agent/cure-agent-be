...
| 인증 | HttpOnly 쿠키 + CSRF 가드 + refresh rotation·재사용 감지 |
...
- e2e 필수 시나리오:
  ...
  - refresh rotation·재사용 감지 → family 전체 폐기
...
4. Guideline·Evidence + 인제스트 (exact search)
5. Conversation·Message + SSE + LLM 게이트웨이(4단 방어 포함)
6. FE 공통: http 전송 계층(단일화 refresh) + app-shell + 로그인
7. 지침 질의 3단 화면
8. Patient + PatientProfileSnapshot (필드 암호화·스코프 강제 포함)
9. ClinicalGuidance + 의료인 검토
10. History
11. 운영 마감: 실시간 알림 연결, e2e 크로스테넌트·SSE 복구 시나리오, gitleaks·contract CI 확인
