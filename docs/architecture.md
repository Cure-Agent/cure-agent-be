...
**환자 대화 메시지의 스냅샷에는 새 축이 없다 (docs/specs/53)**: `messages.patientSnapshotId`가 가리키는 스냅샷은 언제나 `conversations.patientId`의 것이라 환자 삭제가 이미 그 대화를 예약하고(위 §34 연쇄), 파기도 기존 순서(메시지 → 스냅샷)로 그대로 풀린다.
...
**환자 기록을 딛는 검색 입력 (docs/specs/53·51):**

- **환자 한 명이 정해진 턴은 그 환자의 진단명을 검색·리랭크 입력에 붙인다** — 환자 고정 대화(PATIENT_GUIDANCE)와 에이전트 복합 경로가 **같은 규칙**이다. 두 경로 모두 질문이 병명을 자연스럽게 생략하는데(「이 환자에게 침 치료를 고려할 때 주의할 점은?」·「CASE-001에게 침 치료해도 돼?」) 그대로면 검색이 흩어지고, 실패는 기권보다 **「다른 질환 근거로 통과」**다(docs/specs/53 실측: 지침 점유 2.67/5 → 4.88/5).
- **붙이는 자리는 §42 번역 뒤**다. 번역 앞에 붙이면 한글 진단명이 언어 판정을 흔들어 짧은 영문 질문이 번역을 건너뛴다. 리랭커는 채팅처럼 **사용자가 쓴 질문**을 보되 같은 진단명을 붙여 받는다(§29) — 병명이 빠진 질문으로는 「이 환자에게 관련 있나」를 잴 수 없다.
- **언급 판정을 두지 않는다** — 질문이 이미 그 진단명을 담고 있어도 붙인다. 판정을 두면 표기가 갈리는 질문(「ADHD」 ↔ 「주의력결핍 과잉행동장애」)에서 어차피 빗나가고, 정규화·부분문자열 예외가 두 경로에 새로 생긴다. 측정 결과는 항상 붙이는 쪽과 같았다.
- **붙이는 필드는 진단명뿐이다.** 투약·알레르기·임상 메모는 생성 프롬프트에만 간다 — 임상 메모는 자유 텍스트 4,000자라 검색어로 쓰면 질문을 덮는다. **생성 LLM이 받는 질문에는 덧붙지 않는다**(부착은 검색·리랭크 입력에만 있다).
- **`generation_runs.search_question`에는 붙인 그대로 남긴다** — 기록만으로 실제 검색 입력이 재현된다. 암호화 필드의 값이 평문 컬럼에 실리지만 그 행들의 수명은 환자 삭제 연쇄(§5.7)가 환자에 묶는다.
- **기록은 검색 **전에** 한 번 읽어 그 턴에 고정한다** — 검색의 진단명·생성 프롬프트의 프로필·참고안이 모두 같은 스냅샷 하나를 딛는다(§9). 검색 결과가 환자 기록에 좌우되므로 **기권한 턴에도** 「어느 기록을 봤나」가 남아야 붙인 진단명을 되짚을 수 있다(§5.7 재현성).
